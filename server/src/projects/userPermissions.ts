...
import { IUserGroup, IUserRole } from './statamicProject';
	...
	try {
		...
		for (let i = 0; i < documentRoles.length; i++) {
			...
			if (typeof roleDetails !== 'undefined' && roleDetails !== null) {
				if (typeof roleDetails.title !== 'undefined' && roleDetails.title !== null) {
	...
	try {
		...
		for (let i = 0; i < documentGroups.length; i++) {
			...
			if (typeof groupDetails !== 'undefined' && groupDetails !== null) {
				if (typeof groupDetails.title !== 'undefined' && groupDetails.title !== null) {

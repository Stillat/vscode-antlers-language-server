...
export class LanguageParser {
		...
		for (let i = 0; i < tokenCount; i++) {
			...
			if (token instanceof LanguageOperatorConstruct) {
				if (token.content.includes('.')) {
					...
					i += 1;
					continue;
				} else if (token.content == LanguageOperatorRegistry.ARR_PLUCK_INTO) {
					if (i + 1 >= tokenCount) {
						token.pushError(AntlersError.makeSyntaxError(
					...
					if (nextT instanceof VariableNode == false) {
						nextT.pushError(AntlersError.makeSyntaxError(
							AntlersErrorCodes.TYPE_PLUCK_INTO_INVALID_VARIABLE_TARGET,
							nextT,
							'Unexpected [' + TypeLabeler.getPrettyTypeName(nextT) + ']. Expecting [T_VAR].'
						));
						continue;
					}
					...
					const predicateWrapperT = tokens[i + 2];

					if (predicateWrapperT instanceof LogicGroup == false) {
						predicateWrapperT.pushError(AntlersError.makeSyntaxError(
							AntlersErrorCodes.TYPE_PLUCK_INTO_INVALID_PREDICATE_VALUE,
							predicateWrapperT,
							'Unexpected [' + TypeLabeler.getPrettyTypeName(predicateWrapperT) + '] while parsing pluck_into; expecting [T_LOGIC_GROUP].'
						));
						continue;
					}

					const predicateWrapper: LogicGroup = predicateWrapperT as LogicGroup;

					if (predicateWrapper.nodes.length == 0) {
						nextT.pushError(AntlersError.makeSyntaxError(
					...
					const innerWrapperNodeT = predicateWrapper.nodes[0];

					if (innerWrapperNodeT instanceof SemanticGroup == false) {
						nextT.pushError(AntlersError.makeSyntaxError(
							AntlersErrorCodes.TYPE_PLUCK_INTO_EMPTY_LOGIC_GROUP,
							nextT,
							'Unexpected empty [T_LOGIC_GROUP].'
					...
					const innerWrapperNode: SemanticGroup = innerWrapperNodeT as SemanticGroup;
					const innerNodes = innerWrapperNode.nodes;
					...
					let isDynamicNames = true;
					const innerNodeCount = innerNodes.length;

					for (let c = 0; c < innerNodeCount; c++) {
						if (innerNodes[c] instanceof ScopeAssignmentOperator) {
							isDynamicNames = false;

							if (c != 3) {
								innerNodes[c].pushError(AntlersError.makeSyntaxError(
					...
					const next = nextT as VariableNode;
					const tupleScopedGroup = new TupleScopedLogicGroup();
					tupleScopedGroup.target = next;

					if (!isDynamicNames) {
						tupleScopedGroup.item1 = innerNodes[0];
						tupleScopedGroup.item2 = innerNodes[1];
						const tsgItem = innerNodes.pop() as AbstractNode;
						tupleScopedGroup.nodes.push(tsgItem);
						...
						if (innerNodeCount == 0 || innerNodes[0] instanceof LogicGroup == false) {
							...
							if (innerNodes[0] instanceof LogicGroup) {
								const tLg = innerNodes[0] as LogicGroup;
								tupleScopedGroup.nodes = tLg.nodes;
							}
						}
					...
					newTokens.push(token);
					newTokens.push(tupleScopedGroup);

					if (i + 3 < tokenCount && i + 4 < tokenCount && tokens[i + 3] instanceof VariableNode) {
						const varName = tokens[i + 3] as VariableNode;

						if (varName.name != LanguageKeywords.ScopeAs) {
							varName.pushError(AntlersError.makeSyntaxError(
	...
	private createTernaryGroups(tokens: AbstractNode[]) {
		...
		for (let i = 0; i < tokenCount; i++) {
			...
			if (node instanceof InlineTernarySeparator) {
				...
				const targetJumpIndex = (separator.foundAt ?? 0) + 2;
	...
	private groupNodesByType(nodes: AbstractNode[], type: any) {
		...
		for (let i = 0; i < nodeCount; i++) {
			...
			if (i > 0 && node instanceof type) {
				const left: AntlersNode[] = [];
				const poppedLeft = newNodes.pop() as AntlersNode;
	...
	private insertAutomaticStatementSeparators(tokens: AbstractNode[]) {
		...
		for (let i = 0; i < tokenCount; i++) {
			...
			if (LanguageParser.isAssignmentOperatorNode(thisToken) && i > 1) {
				const check = tokens[i - 2];

				if ((check instanceof StatementSeparatorNode) == false) {
					const replace = adjustedTokens.pop() as AbstractNode;
					adjustedTokens.push(new StatementSeparatorNode());
					adjustedTokens.push(replace);
